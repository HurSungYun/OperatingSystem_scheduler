...
int wlan_priv_cmd(struct net_device *dev, struct ifreq *ifr)
{
	...
	*ptr = 0;
	...
	if ((strcmp(command, "COUNTRY") == 0) && (strlen(ptr) == 2)) {
		if (strcmp(ptr, "US") == 0) {
			rd_size = sizeof(wlan_ieee80211_regdomain) +
					sizeof(struct ieee80211_reg_rule);
			wlan_cmd_set_regdom(0, (unsigned char *)(&wlan_regdom_11),
					rd_size);
		} else {
			rd_size = sizeof(wlan_ieee80211_regdomain) + 2 *
					sizeof(struct ieee80211_reg_rule);
			wlan_cmd_set_regdom(0, (unsigned char *)(&wlan_regdom_12_13),
					rd_size);
		}
	} else {
