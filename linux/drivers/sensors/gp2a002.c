...
static int gp2a_leda_onoff(struct gp2a_data *gp2a, int power)
{
	...
	if (ret < 0)
		pr_err("[SENSOR] %s, error for direction\n", __func__);
...
static int gp2a_cal_mode_read_file(struct gp2a_data *gp2a)
{
	...
	if (IS_ERR(cal_mode_filp)) {
		...
		if (err != -ENOENT)
			pr_err("[SENOSR] %s, Can't open cal_mode file\n",
				__func__);
		set_fs(old_fs);
		return err;
	}
	...
	if (err != sizeof(u8)) {
		pr_err("[SENOSR] %s, Can't read the cal_mode from file\n",
			__func__);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1")) {
		...
		pr_err("[SENOSR] %s, invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
	...
	if (err < 0) {
		pr_err("[SENOSR] %s, prox_cal_write() failed\n", __func__);
		return err;
	}
...
static int gp2a_regulator_onoff(struct device *dev, bool onoff)
{
	...
	if (onoff) {
		...
		if (ret) {
			pr_err("[SENOSR] %s: enable vdd failed (%d)\n",
				__func__, ret);
			return ret;
		}
	} else {
		...
		if (ret) {
			pr_err("[SENOSR] %s: disable vdd failed (%d)\n",
				__func__, ret);
			return ret;
		}
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (err) {
		pr_err("[SENOSR] %s, kstrtoint failed.", __func__);
		return -EINVAL;
	}
	if (value != 0 && value != 1) {
		pr_err("[SENOSR] %s, wrong value(%d)\n", __func__, value);
		return -EINVAL;
	}
	...
	if (gp2a->power_state != value) {
		...
		pr_err("[SENOSR] %s, wrong cmd for enable\n", __func__);
	}
