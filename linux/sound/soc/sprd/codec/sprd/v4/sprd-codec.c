...
static int pin_switch_bt_iis_sys_sel_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
...
static int pin_switch_bt_iis_sys_sel_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pinmap_set(REG_PIN_CTRL2, val);

	return 1;
}

static int pin_switch_iis0_sys_sel_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
...
static int pin_switch_iis0_sys_sel_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	unsigned int v = ucontrol->value.integer.value[0];
	unsigned int shift = 6;
	...
	pinmap_set(REG_PIN_CTRL3, val);

	return 1;
}

static int pin_switch_bt_iis_con_switch_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
...
static int pin_switch_bt_iis_con_switch_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
	...
	unsigned int shift = mc->shift;
	unsigned int mask = mc->max;
	unsigned int v = (ucontrol->value.integer.value[0] & mask);
	unsigned int val = 0;
...
static const char *pin_switch_bt_iis_sys_sel_function[] = {
	"cp0_iis0", "cp0_iis1", "cp0_iis2", "cp0_iis3",
	"cp1_iis0", "cp1_iis1", "cp1_iis2", "cp1_iis3",
	"ap_iis0", "ap_iis1", "ap_iis2", "ap_iis3",
	"vbc_iis", "rsv_d", "rsv_e", "rsv_f" };

static const struct soc_enum bt_iis_sys_sel_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pin_switch_bt_iis_sys_sel_function),
				pin_switch_bt_iis_sys_sel_function);

static const char *pin_switch_iis0_sys_sel_function[] = {
	"ap_iis0", "cp0_iis0", "cp1_iis0", "cp2_iis0", "vbc_iis0" };

static const struct soc_enum iis0_sys_sel_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pin_switch_iis0_sys_sel_function),
			    pin_switch_iis0_sys_sel_function);
...
static const struct snd_kcontrol_new sprd_codec_snd_controls[] = {
	...
	SOC_ENUM_EXT("BT IIS pin select", bt_iis_sys_sel_enum,
			pin_switch_bt_iis_sys_sel_get,
			pin_switch_bt_iis_sys_sel_put),

	SOC_ENUM_EXT("IIS0 pin select", iis0_sys_sel_enum,
			pin_switch_iis0_sys_sel_get,
			pin_switch_iis0_sys_sel_put),

	SOC_SINGLE_EXT("BT IIS con switch", REG_PIN_CTRL3, 30, 1, 0,
			pin_switch_bt_iis_con_switch_get,
			pin_switch_bt_iis_con_switch_put),
};
