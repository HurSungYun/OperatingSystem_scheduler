...
static unsigned long s_vpu_reg_phy_addr = VPU_REG_BASE_ADDR;
...
#define REG_PMU_APB_CODEC_CFG REG_PMU_APB_PD_MM_CODEC_CFG
#define AUTO_SHUTDOWN_EN           BIT_PD_MM_CODEC_AUTO_SHUTDOWN_EN
#define FORCE_SHUTDOWN                BIT_PD_MM_CODEC_FORCE_SHUTDOWN
#else
#define REG_PMU_APB_CODEC_CFG REG_PMU_APB_PD_CODEC_TOP_CFG
#define AUTO_SHUTDOWN_EN           BIT_PD_CODEC_TOP_AUTO_SHUTDOWN_EN
#define FORCE_SHUTDOWN                BIT_PD_CODEC_TOP_FORCE_SHUTDOWN
#endif
...
static int vpu_power_on()
{
    __raw_writel(AUTO_SHUTDOWN_EN | __raw_readl(REG_PMU_APB_CODEC_CFG), REG_PMU_APB_CODEC_CFG);
    __raw_writel((~FORCE_SHUTDOWN) & __raw_readl(REG_PMU_APB_CODEC_CFG), REG_PMU_APB_CODEC_CFG);
    return 0;
}
...
#if 0
static int vpu_set_clk_by_register()
{
    __raw_writel((~(1<<25))&__raw_readl(REG_PMU_APB_PD_MM_TOP_CFG), REG_PMU_APB_PD_MM_TOP_CFG);  //0x402b_001c  (&0xfdff_ffff)
    __raw_writel((0x02000000)|__raw_readl(REG_AON_APB_APB_EB0), REG_AON_APB_APB_EB0);  //0x402e_0000  (|0x0200_0000)

    __raw_writel(__raw_readl(REG_AON_APB_APB_EB1) | BIT_CODEC_EB, REG_AON_APB_APB_EB1); //0x402e_0004  (|0x0000_4000)
    __raw_writel(__raw_readl(REG_CODEC_AHB_CLOCK_SEL) | 0x0333, REG_CODEC_AHB_CLOCK_SEL); //6200_0008 (|0x0333)
    __raw_writel(__raw_readl(REG_CODEC_AHB_CKG_ENABLE) | 0x03, REG_CODEC_AHB_CKG_ENABLE); //6200_0004 (|0x03)
...
static int vpu_clk_free(vpu_drv_context_t* vpu_context)
{
    if (vpu_context->clk_coda7_apb) {
        //clk_disable_unprepare(vpu_context->clk_coda7_apb);
        clk_disable(vpu_context->clk_coda7_apb);
    }

    if (vpu_context->clk_coda7_axi) {
        //clk_disable_unprepare(vpu_context->clk_coda7_axi);
        clk_disable(vpu_context->clk_coda7_axi);
    }

    if (vpu_context->clk_coda7_cc) {
        //clk_disable_unprepare(vpu_context->clk_coda7_cc);
