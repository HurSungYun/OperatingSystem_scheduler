...
static void extra_notify_work(struct work_struct *data)
{
	struct ext_state_work *state_work =
		container_of(data, struct ext_state_work, ext_work);
	...
	blocking_notifier_call_chain
		(&n->extra_notifier, state_work->event, &state_work->enable);

	kfree(state_work);
	return;
}

static void send_extra_notify(unsigned long event, int enable)
{
	struct ext_state_work *state_work;
	...
	state_work = kmalloc(sizeof(struct ext_state_work), GFP_ATOMIC);
	if (!state_work) {
		pr_err("%s:unable to allocate state_work\n", __func__);
		return;
	}

	INIT_WORK(&state_work->ext_work, extra_notify_work);
	state_work->event = event;
	state_work->enable = enable;
	queue_work(u_notify->notifier_wq, &state_work->ext_work);
				...
				unsigned long event, int enable)
{
	...
	if (type & NOTIFY_EVENT_EXTRA)
		send_extra_notify(event, enable);
