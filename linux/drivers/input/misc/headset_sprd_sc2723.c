...
#define ANA_CDC3 (0x0028)
#define ANA_HDT0 (0x0068)
#define ANA_HDT2 (0x0070)
#define ANA_HDT1 (0x006C)
#define ANA_STS0 (0x0074)
#define DIG_STS0 (0x00A0)
#define DIG_STS1 (0x00A4)
#define DIG_CFG3 (0x0098)
#define DIG_CFG4 (0x009C)
...
/*add by summer*/
#define SPRD_IMPD_PHY_REG_EN(x)   do{sprd_impd_reg_mclk(x);}while(0)
#define SPRD_IMPD_PHY_MCLK_M(x)   do{sprd_impd_mclk(x);}while(0)
#define SPRD_IMPD_SOFT_RESET_M()  do{sprd_impd_soft_reset();}while(0)

#define ANA_ARM_MODULE_EN 	(ANA_REGS_GLB_BASE+0x0000)
#define ANA_IMPD_ADC_EN 	BIT(12)
#define ANA_AUD_EN		BIT(4)

#define ANA_ARM_CLK_EN		(ANA_REGS_GLB_BASE+0x0004)
#define ANA_CLK_IMPD_ADC_EN	BIT(10)

#define ANA_AUDIO_CTRL1  	(ANA_REGS_GLB_BASE+0x0100)
#define ANA_IMPD_ADC_SOFT_RST 	BIT(7)


#define AUDIO_CTRL 			(ANA_IMPD_BASE+0x0000)
#define AUDIO_DATA			(ANA_IMPD_BASE+0x0004)
/*belong register ANA_HDT2*/
#define AUD_IMPD_BUF_EN		BIT(14)
#define AUD_IMPD_ADC_EN		BIT(12)
#define AUD_IMPD_ADC_RST	BIT(11)
#define AUD_IMPD_CUR_EN		BIT(15)

/*belong register ANA_STS0*/
#define AUD_IMPD_ADD_DONE	BIT(14)
#define AUD_IMPD_SUB_DONE	BIT(13)

/*belong register AUDIO_CTRL*/
#define IMPD_AUDIO_CHARGE_INT_RAW		BIT(3)
#define IMPD_AUDIO_CHARGE_INT_EN		BIT(7)
#define IMPD_AUDIO_DISCHARGE_INT_EN		BIT(6)
#define IMPD_AUDIO_WRITE_STATUS			BIT(11)
#define IMPD_AUDIO_CHARGE_INT_STS		BIT(1)
#define IMPD_AUDIO_DISCHARGE_INT_STS	BIT(0)
#define IMPD_AUDIO_CHARGE_INT_CLR		BIT(5)
#define IMPD_AUDIO_DISCHARGE_INT_CLR	BIT(4)



#define ANA_REG_OR(addr,value)	\
do{	\
	uint32_t temp;	\
	temp = sci_adi_read(addr);	\
	temp = temp | (uint32_t)(value);	\
	sci_adi_raw_write(addr, temp);  \
   }while(0)

#define ANA_REG_AND(addr,value)	\
do{	\
	uint32_t temp; 	\
	temp = sci_adi_read(addr);	\
	temp = temp & (uint32_t)(value);	\
...
#define ANA_REG_MSK_OR(addr,value,msk)	\
do{	\
	uint32_t temp;	\
	temp = sci_adi_read(addr);	\
	temp = temp & (uint32_t)(~(msk));	\
	temp = temp | (uint32_t)((value)&(msk));	\
	sci_adi_raw_write(addr, temp);  \
   }while(0)


static __inline void sprd_impd_reg_mclk(uint32_t en){
	if(en)
	{
		/* ANA_ARM_MODULE_EN belong global register base is 0x40038800 */
		ANA_REG_OR(ANA_ARM_MODULE_EN,(ANA_IMPD_ADC_EN | ANA_AUD_EN));
	}
	else
	{
		ANA_REG_AND(ANA_ARM_MODULE_EN,(~(ANA_IMPD_ADC_EN)));
	}
}

static __inline void sprd_impd_mclk(uint32_t en){
	if(en)
	{
		ANA_REG_OR(ANA_ARM_CLK_EN,ANA_CLK_IMPD_ADC_EN);//26M
	}
	else
	{
		ANA_REG_AND(ANA_ARM_CLK_EN,(~(ANA_CLK_IMPD_ADC_EN)));
	}

}

static __inline void sprd_impd_soft_reset(void){
	int i;
	ANA_REG_OR(ANA_AUDIO_CTRL1,ANA_IMPD_ADC_SOFT_RST);
	for(i=0;i<10;i++)
	ANA_REG_AND(ANA_AUDIO_CTRL1,~(ANA_IMPD_ADC_SOFT_RST));
}
...
static volatile int ear_adc_value = 0;
...
static int headset_impd_int(void);
uint32_t __sprd_codec_impd_mcu_test(void);

static int headset_impd_int(void){

	/*1.set the global register*/
	SPRD_IMPD_PHY_MCLK_M(1);
	SPRD_IMPD_PHY_REG_EN(1);
	/*2.set value for register*/
	headset_reg_set_val(HEADMIC_DETECT_REG(ANA_CDC3), 0x8000, 0x0000, 0);
...
uint32_t __sprd_codec_impd_mcu_test(void){
	...
	/*0x73C8 (0.8ua 4ms 1k 10k) 0x72A8 (0.8ua 1ms 2k 10k) 0x7080 (1.6ua 1ms 4k 5k) 0x76F8 (0.2ua 1ms 0.5k 40k)*/
	PRINT_INFO("summer.wang impd_mcu 01 ANA_HDT2 = 0x%08x\n",sci_adi_read(HEADMIC_DETECT_REG(ANA_HDT2)));
	headset_reg_set_val(HEADMIC_DETECT_REG(ANA_HDT2), 0x76F8, 0xffff, 0);
	PRINT_INFO("summer.wang impd_mcu 02 ANA_HDT2 = 0x%08x\n",sci_adi_read(HEADMIC_DETECT_REG(ANA_HDT2)));

	headset_reg_set_bit(AUDIO_CTRL, IMPD_AUDIO_CHARGE_INT_CLR | IMPD_AUDIO_DISCHARGE_INT_CLR);
	/*1.enable BUF_EN and ADC_EN*/
	headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_BUF_EN);
	headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_ADC_EN);

	/*2.ADC_RST*/
	headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_ADC_RST);
	msleep(10);//delay 10ms
	headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_ADC_RST);

	/*3.CUR_EN = 1 and soft reset*/
	headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_CUR_EN);
	SPRD_IMPD_SOFT_RESET_M();

	/*4.wait add_done = 0*/
	while(sci_adi_read(HEADMIC_DETECT_REG(ANA_STS0)) & AUD_IMPD_ADD_DONE){
		//set the max wait times 32
		if(timewait++<0x20){
			msleep(10);
		}
		else{
	...
	/*5.wait sub_done = 0*/
	while(sci_adi_read(HEADMIC_DETECT_REG(ANA_STS0)) & AUD_IMPD_SUB_DONE){
		if(timewait++<0x20){
			msleep(10);
		}
		else{
	...
	/*6.check add_done(bit 14) = 0 and sub_done(bit 13) = 0*/
	PRINT_INFO("summer.wang impd_mcu step6 ANA_STS0= 0x%08x\n",sci_adi_read(HEADMIC_DETECT_REG(ANA_STS0)));

	/*check add_done_int_raw(bit 3) = 0 and sub_done_int_raw(bit 2) = 0*/
	PRINT_INFO("summer.wang impd_mcu step6 AUDIO_CTRL = 0x%08x\n",sci_adi_read(AUDIO_CTRL));
	...
	/*8.delay 250ms and wait add_done_int_raw = 1*/
	msleep(250);
	while(!(sci_adi_read(AUDIO_CTRL) & IMPD_AUDIO_CHARGE_INT_RAW)){
		if(timewait++<0x20){
			msleep(10);
		}
		else{
	...
	/*check add_done_int_raw(bit 3) = 1*/
...
*/
	...
	/*11. wait sub_done = 1 and chek sub_done_int_raw = 1*/
	while(!(sci_adi_read(HEADMIC_DETECT_REG(ANA_STS0)) & AUD_IMPD_SUB_DONE)){
		if(timewait++<0x20){
			msleep(10);
		}
	...
	/*check sub_done_int_raw*/
	PRINT_INFO("summer.wang impd_mcu step11 AUDIO_CTRL = 0x%08x\n",sci_adi_read(AUDIO_CTRL));


	/*12.disable BUF_EN and ADC_EN*/
	headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_BUF_EN);
	headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT2),AUD_IMPD_ADC_EN);


	val = sci_adi_read(AUDIO_DATA);
	PRINT_INFO("summer.wang impd_mcu step12 adc_val = 0x%08x\n",val);
	return val;
}
...
static void headset_detect_work_func(struct work_struct *work)
{
	 ...
	 if(0 == plug_state_last){
		...
		PRINT_INFO("summer wang readtimes = %d\n",readtimes);
		PRINT_INFO("summer wang val_impd= %d\n",val_impd);
		PRINT_INFO("summer wang value_current = %d\n",value_current);
		while((val_impd>4200) && (value_current==1)){
			if(readtimes++<4){
				val_impd = __sprd_codec_impd_mcu_test();
				value_current = gpio_get_value(pdata->gpio_detect);
				PRINT_INFO("summer wang readtimes = %d\n",readtimes);
				...
				headset_type = 1;
				PRINT_INFO("summer wang 3 pole headphone \n");
				break;
			}
		}
		if((readtimes<5) &&(value_current==1))  //val_impd < threshold  and headset piug in --> impd_flag=1
			impd_flag=1;
		else
			impd_flag=0;
		PRINT_INFO("summer wang impd_flag = %d\n",impd_flag);
		readtimes=0;
		SPRD_IMPD_PHY_MCLK_M(0);
		SPRD_IMPD_PHY_REG_EN(0);
	}
...
//#ifdef CONFIG_SPRD_INTERNAL_HEADSET_SUPPORT
		...
		if(impd_flag){
			...
			PRINT_INFO("summer wang  headmicbias power on \n");
		}
		...
		if(impd_flag){
			PRINT_INFO("summer wang headset_type_detect \n");
			headset_type = headset_type_detect(gpio_detect_value_last);
				...
				PRINT_INFO("summer wang headset_type = %d\n", headset_type);
                detect_err_count = 0;
...
#endif
						headmicbias_power_on(&this_pdev->dev, 0);						//add by summer
						PRINT_INFO("summer wang headmicbias_power off  01  3 pole\n");
...
#endif
				headmicbias_power_on(&this_pdev->dev, 0);					//add by summer
				PRINT_INFO("summer wang  headmicbias power off 02  end plug out  \n");
                headset_irq_detect_enable(1, ht->irq_detect);
        } else {
...
out:
		headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT1),BIT(15));
		headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_PMU0),BIT(4));
		...
		PRINT_INFO("summer wang  headset_detect_work_func end \n");
...
static int headset_detect_probe(struct platform_device *pdev)
{
	...
	struct device *earjack;
        ...
        sci_adi_write(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_AUD_EN, BIT_ANA_AUD_EN);//arm base address:0x40038800 for register accessable
        ana_sts0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+ANA_STS0);//arm base address:0x40038600
		headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_HDT1),BIT(15)); //add by summer
		headset_reg_set_bit(HEADMIC_DETECT_REG(ANA_PMU0),BIT(4));  //add by summer
