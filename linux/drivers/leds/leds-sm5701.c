...
struct SM5701_leds_data {
        ...
        unsigned int flashen;
        unsigned int flashset;
...
static int sm5701_leds_parse_dt(struct SM5701_leds_data *chip)
{
	struct device_node *np = of_find_node_by_name(NULL, "leds_sm5701");
	int ret = 0;
	...
	if (np == NULL) {
		...
		chip->flashen = of_get_named_gpio(np, "flashen-gpio", 0);
		chip->flashset = of_get_named_gpio(np, "flashtorch-gpio", 0);

		pr_info("%s: SM5701_parse_dt flashen: %d\n", __func__, chip->flashen);
		pr_info("%s: SM5701_parse_dt flashset: %d\n", __func__, chip->flashset);

		ret = gpio_request(chip->flashen, NULL);
		if (ret < 0)
			printk(KERN_WARNING"[FLASH] FLASHEN request failed.%d\n", ret);
		else
			gpio_direction_output(chip->flashen, 0);

		ret = gpio_request(chip->flashset, NULL);
		if (ret < 0)
			printk(KERN_WARNING"[FLASH] FLASHSET %d\n", ret);
		else
			gpio_direction_output(chip->flashset, 0);
	}
...
static int leds_sm5701_probe(struct platform_device *pdev)
{
        ...
        /* To control FLED releated GPIOs */
        err = sm5701_leds_parse_dt(chip);
        if (err < 0)
                dev_err(chip->dev, "Failed to control GPIOs\n");
