...
#include "lcd/s6e8aa5x01_dimming.h"
#define ENABLE_CLK_HS_ON_INIT	/* video sync start before init command*/
#endif
...
struct panel_operations {
	...
	int32_t (*panel_set_brightness)(struct sprdfb_device *fb_dev,
				uint16_t brightness);
	...
	uint32_t (*panel_get_color_coordinates)(struct sprdfb_device *fb_dev, uint32_t *co_ord);
	void (*panel_send_mdnie_cmds)(struct sprdfb_device *fb_dev, struct mdnie_command *seq, u32 len);
#endif
	int32_t (*panel_initialize)(struct sprdfb_device *fb_dev, struct device *dev);
	int32_t (*panel_get_type)(char *buf, unsigned int panel_id);
	int32_t (*panel_power_on)(struct sprdfb_device *dev, bool enable);
	int32_t (*panel_read_offset)(struct panel_spec *self);
};
