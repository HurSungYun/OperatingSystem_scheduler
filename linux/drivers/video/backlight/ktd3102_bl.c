...
static int temp[MAX_BRT_VALUE_IDX][2] = {
	{0, 5},
	{10, 6},
	{60, 16},
	{100, 29},
...
static int ktd3102_backlight_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (outdoor_mode_en) {
			...
			if (ret == 0) {
				...
				ginfo->outdoor_value.brightness = temp[BRT_VALUE_HBM][0];
				ginfo->outdoor_value.tune_level = temp[BRT_VALUE_HBM][1];
			}
		}
		...
		if (ret < 0) {
			for (i = 0; i < MAX_BRT_VALUE_IDX; i++) {
				ginfo->range[i].brightness = temp[i][0];
				ginfo->range[i].tune_level = temp[i][1];
			}
		} else {
			for (i = 0; i < MAX_BRT_VALUE_IDX; i++) {
	...
	bd = backlight_device_register(KTD3102_BL_NAME, &pdev->dev, bl_info,
