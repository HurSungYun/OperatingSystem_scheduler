...
#include <linux/input/zinitix_bt541_ts.h>
...
#define CHIP_ON_DELAY				20 /*15*/ /*ms*/
#define FIRMWARE_ON_DELAY			60 /*40*/ /*ms*/
...
static void clear_report_data(struct bt541_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.button_num; i++) {
		if (info->button[i] == ICON_BUTTON_DOWN) {
			...
			input_report_key(info->input_tkey, BUTTON_MAPPING_KEY[i], 0);
			reported = true;
	...
	if (zinitix_bit_test(info->touch_info.status, BIT_ICON_EVENT)) {
		...
		for (i = 0; i < info->cap_info.button_num; i++) {
			if (zinitix_bit_test(info->icon_event_reg,
				...
				input_report_key(info->input_tkey, BUTTON_MAPPING_KEY[i], 1);
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
		...
		const struct i2c_device_id *i2c_id)
{
	...
	input_set_abs_params(info->input_dev, ABS_X, 0, info->cap_info.MaxX, 0, 0);
	input_set_abs_params(info->input_dev, ABS_Y, 0, info->cap_info.MaxY, 0, 0);
	...
	ret = input_register_device(info->input_tkey);
	if (ret) {
...
#ifdef CONFIG_PM_SLEEP
	info->input_dev->open = bt541_ts_open;
	info->input_dev->close = bt541_ts_close;
#endif
...
err_power_sequence:
	input_free_device(info->input_dev);
	input_free_device(info->input_tkey);
err_alloc:
	kfree(info);
err_gpio_alloc:
...
static int bt541_ts_remove(struct i2c_client *client)
{
	...
	if (gpio_is_valid(pdata->gpio_ldo_en) != 0)
		gpio_free(pdata->gpio_ldo_en);

	tsp_charger_status_cb = NULL;

	input_unregister_device(info->input_dev);
	input_unregister_device(info->input_tkey);
	input_free_device(info->input_dev);
	up(&info->work_lock);
	kfree(info);
