...
uint32_t conti_base_addr;
uint32_t sep_base_addr;
uint32_t ana_size;
		...
		, char __user *buffer, size_t len, loff_t *offset)
{
	...
	/*printk("chip_id 0x%08x, ana_chip_id 0x%08x",
		sci_get_chip_id(), sci_get_ana_chip_id());*/
...
uint32_t hwconfig_read_reg(uint32_t addr)
{
	pr_debug("[hwconfig], hwconfig_read_reg 0x%08x\n", addr);
	return readl_relaxed((void __iomem *)addr);
}

uint32_t hwconfig_read_anareg(uint32_t addr)
{
	uint32_t real_addr = (u32)SPRD_ADI_BASE+addr;
...
static int trigger_conti_reg_check(char *buffer)
{
	...
	if (ptr) {
		...
		/*printk("[hwconfig] trigger_conti_reg_
			check conti_reg_type %d\n", conti_reg_type);*/
	}
	ptr = strsep(&buffer, ",");
	if (ptr) {
		phy_addr = simple_strtoul(ptr, NULL, 16);
		pr_debug("[hwconfig]conti phy 0x%08x\n", phy_addr);
	}
	ptr = strsep(&buffer, ",");
	if (ptr) {
		size = simple_strtoul(ptr, NULL, 16);
		pr_debug("[hwconfig]conti size 0x%08x\n", size);
	}
	if (conti_reg_type == normal_type) {
	...
	pr_debug("[hwconfig]conti addr 0x%08x\n", conti_base_addr);

	ptr = strsep(&buffer, ",");
	if (ptr) {
		reg_base_addr = simple_strtoul(ptr, NULL, 16);
		/*pr_debug("[hwconfig] trigger_conti_reg_
			check reg_base_addr 0x%08x
...
static int trigger_sep_reg_check(char *buffer)
{
	...
	if (ptr) {
		phy_addr = simple_strtoul(ptr, NULL, 16);
		pr_debug("[hwconfig]sep phy_addr 0x%08x\n", phy_addr);
	}
	ptr = strsep(&buffer, ",");
	if (ptr) {
		size = simple_strtoul(ptr, NULL, 16);
		pr_debug("[hwconfig]sep size 0x%08x\n", size);
...
{
	char buf[64] = {0};
	sprintf(buf, "0x%08x,0x%08x,", sci_get_chip_id()>>16,
			sci_get_ana_chip_id()>>16);
	pr_debug("[hwconfig], %s\n", buf);
